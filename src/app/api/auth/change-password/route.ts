...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const userData = stmtUser.get(userId) as
      | (Pick<User, "id"> & { hashedPassword?: string })
      | undefined;

    if (!userData || !userData.hashedPassword) {
      console.warn(`API Change Password: User not found or no hashed password for ID: ${userId}`);
      return NextResponse.json(
        { error: "Invalid user or unable to verify current password." },
        { status: 401 }
      );
    }
    ...
    const stmtUpdatePassword = db.prepare(
      "UPDATE Users SET hashedPassword = ?, updatedAt = ? WHERE id = ?"
    );
    const result = stmtUpdatePassword.run(newHashedPassword, now, userId);

    if (result.changes === 0) {
      console.error(
        `API Change Password: Failed to update password in DB for user ID: ${userId}. No rows affected.`
      );
      // This case should ideally not happen if the user was fetched correctly.
      return NextResponse.json(
        { error: "Failed to update password. User not found or no change needed." },
        { status: 500 }
      );
    }
    ...
    if (
      ...
      console.error(
        `API Change Password: SQLite Error - Code: ${error.code}, Message: ${error.message}`
      );
      return NextResponse.json(
