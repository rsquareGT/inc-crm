...
export async function DELETE(
  request: NextRequest,
  { params }: { params: { dealId: string; noteId: string } }
) {
  try {
    ...
    if (!dealData) {
      return NextResponse.json(
        { error: "Deal not found or not authorized for note deletion" },
        { status: 404 }
      );
    }

    const noteStmt = db.prepare(
      "SELECT content FROM Notes WHERE id = ? AND dealId = ? AND organizationId = ?"
    );
    const noteToDelete = noteStmt.get(noteId, dealId, organizationId) as
      | { content: string }
      | undefined;
