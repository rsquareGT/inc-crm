...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const stmtUser = db.prepare(
      "SELECT id, organizationId, email, hashedPassword, firstName, lastName, profilePictureUrl, role, createdAt, updatedAt FROM Users WHERE email = ?"
    );
    ...
    console.log(
      `API Login: refresh_token cookie set with maxAge ${refreshTokenCookieMaxAge}s for path /api/auth/refresh-token.`
    );
    ...
    if (
      ...
      (error as any).code.startsWith("SQLITE_")
    ) {
      console.error(
        `API Login: SQLite Error - Code: ${(error as any).code}, Message: ${error.message}`
      );
      return NextResponse.json(
