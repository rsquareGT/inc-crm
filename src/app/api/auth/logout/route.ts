  ...
  try {
    ...
    if (refreshTokenValue && db) {
        ...
        if (dbTokenRecordId) {
            ...
            console.warn('API Logout: Refresh token from cookie not found in DB or hash mismatch. No DB record deleted.');
        }
    } else if (refreshTokenValue && !db) {
        console.warn('API Logout: Refresh token present but DB connection unavailable. Cannot delete from DB.');
    }
