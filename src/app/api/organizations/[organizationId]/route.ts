...
export async function GET(
  ...
  try {
    ...
    if (!requestingOrgId) {
      console.warn(
        `API GET /api/organizations/${organizationId}: Unauthorized: Organization ID missing from session.`
      );
      return NextResponse.json(
        { error: "Unauthorized: Organization ID missing from session." },
        { status: 401 }
      );
    }
    ...
    if (organizationId !== requestingOrgId) {
      console.warn(
        `API GET /api/organizations/${organizationId}: Forbidden: User from org ${requestingOrgId} trying to access org ${organizationId}.`
      );
      return NextResponse.json(
        { error: "Forbidden: You can only view your own organization." },
        { status: 403 }
      );
    }
    ...
    if (!db) {
      console.error(
        `API GET /api/organizations/${organizationId}: Database connection is not available`
      );
...
export async function PUT(
  ...
  try {
    ...
    if (!requestingUserId || !requestingOrgId || !userRole) {
      console.warn(
        `API PUT /api/organizations/${targetOrgId}: Unauthorized: User session details missing.`
      );
      return NextResponse.json(
        { error: "Unauthorized: User session details missing." },
        { status: 401 }
      );
    }
    ...
    if (userRole !== "admin") {
      console.warn(
        `API PUT /api/organizations/${targetOrgId}: Forbidden: User ${requestingUserId} (role: ${userRole}) is not an admin.`
      );
      return NextResponse.json(
    ...
    if (targetOrgId !== requestingOrgId) {
      console.warn(
        `API PUT /api/organizations/${targetOrgId}: Forbidden: Admin ${requestingUserId} from org ${requestingOrgId} trying to update different org ${targetOrgId}.`
      );
      return NextResponse.json(
        { error: "Forbidden: Admins can only update their own organization." },
        { status: 403 }
      );
    }
    ...
    if (!db) {
      console.error(
        `API PUT /api/organizations/${targetOrgId}: Database connection is not available`
      );
      return NextResponse.json({ error: "Database connection is not available" }, { status: 500 });
    }
    const body = await request.json();
    console.log(`API PUT /api/organizations/${targetOrgId} - Request body:`, body);
    const { name, logoUrl, street, city, state, postalCode, country, currencySymbol, timezone } =
      body;
    ...
    if (name !== undefined && currentOrgData.name !== name) {
      updates.name = name;
      changes.push({ field: "Name", oldValue: currentOrgData.name, newValue: name });
    }
    if (logoUrl !== undefined && currentOrgData.logoUrl !== logoUrl) {
      updates.logoUrl = logoUrl || null;
      changes.push({
        field: "Logo URL",
        oldValue: currentOrgData.logoUrl,
        newValue: logoUrl || null,
      });
    }
    if (street !== undefined && currentOrgData.street !== street) {
      updates.street = street || null;
      changes.push({ field: "Street", oldValue: currentOrgData.street, newValue: street || null });
    }
    if (city !== undefined && currentOrgData.city !== city) {
      updates.city = city || null;
      changes.push({ field: "City", oldValue: currentOrgData.city, newValue: city || null });
    }
    if (state !== undefined && currentOrgData.state !== state) {
      updates.state = state || null;
      changes.push({ field: "State", oldValue: currentOrgData.state, newValue: state || null });
    }
    if (postalCode !== undefined && currentOrgData.postalCode !== postalCode) {
      updates.postalCode = postalCode || null;
      changes.push({
        field: "Postal Code",
        oldValue: currentOrgData.postalCode,
        newValue: postalCode || null,
      });
    }
    if (country !== undefined && currentOrgData.country !== country) {
      updates.country = country || null;
      changes.push({
        field: "Country",
        oldValue: currentOrgData.country,
        newValue: country || null,
      });
    }
    if (currencySymbol !== undefined && currentOrgData.currencySymbol !== currencySymbol) {
      updates.currencySymbol = currencySymbol || null;
      changes.push({
        field: "Currency Symbol",
        oldValue: currentOrgData.currencySymbol,
        newValue: currencySymbol || null,
      });
    }
    if (timezone !== undefined && currentOrgData.timezone !== timezone) {
      updates.timezone = timezone || null;
      changes.push({
        field: "Timezone",
        oldValue: currentOrgData.timezone,
    ...
    const stmtUpdate = db.prepare(`UPDATE Organizations SET ${setClauses} WHERE id = ?`);
    ...
    if (result.changes === 0) {
      ...
      return NextResponse.json(
        { error: "Organization not found or no changes made during update" },
        { status: 404 }
      );
    }
