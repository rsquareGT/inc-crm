...
export async function DELETE(
  request: NextRequest,
  { params }: { params: { contactId: string; noteId: string } }
) {
  try {
    ...
    if (!contactData) {
      return NextResponse.json(
        { error: "Contact not found or not authorized for note deletion" },
        { status: 404 }
      );
    }
    ...
    const noteStmt = db.prepare(
      "SELECT content FROM Notes WHERE id = ? AND contactId = ? AND organizationId = ?"
    );
    const noteToDelete = noteStmt.get(noteId, contactId, organizationId) as
      | { content: string }
      | undefined;

    const stmt = db.prepare(
      "DELETE FROM Notes WHERE id = ? AND contactId = ? AND organizationId = ?"
    );
    ...
    console.error(
      `API Error deleting note ${params.noteId} for contact ${params.contactId}:`,
      error
    );
