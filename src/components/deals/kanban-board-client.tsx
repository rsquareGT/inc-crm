    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response
          .json()
          .catch(() => ({ error: `Failed to fetch deals: ${response.statusText}` }));
    ...
    return (
      <div className="flex flex-col h-full">
        <PageSectionHeader
          ...
          description="Visually manage your deals through stages."
        >
          <Button disabled>
        ...
        <div className="flex-1 flex gap-4 overflow-x-auto pb-4">
          {DEAL_STAGES.map((stage) => (
            <div
              key={`skeleton-col-${stage}`}
              className={`flex-shrink-0 w-60 bg-secondary/50 rounded-lg p-0.5`}
            >
              <div className="p-1.5">
