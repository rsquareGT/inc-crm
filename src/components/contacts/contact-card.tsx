...
import { MoreHorizontal, Edit, Trash2, ExternalLink, UserCircle, Mail, Phone, Building, FileText } from 'lucide-react';
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
        ...
        <div className="flex justify-between items-start">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              ...
              <DropdownMenuItem onClick={() => onDelete(contact.id)} className="text-destructive hover:!bg-destructive hover:!text-destructive-foreground">
      ...
      <CardContent className="px-4 pb-3 space-y-2 text-sm flex-grow">
        ...
        {company && (
          <div className="flex items-center text-muted-foreground">
            ...
            <Link href={`/companies/${company.id}`} className="text-primary hover:underline truncate" title={company.name}>
              {company.name}
            </Link>
          </div>
        )}
         {contact.description && (
          <div className="flex items-start text-muted-foreground pt-1">
            ...
            <p className="text-xs line-clamp-2" title={contact.description}>{contact.description}</p>
          </div>
        )}
      </CardContent>
      {(tags || []).length > 0 && (
