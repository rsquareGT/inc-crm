          ...
          <div className="flex flex-col lg:flex-row gap-6">
            <div className="w-full lg:w-[65%] space-y-6">
              ...
              <Card>
                ...
                <CardContent><ScrollArea className="h-[250px]">{Array.from({ length: 3 }).map((_, index) => <ActivityItemSkeleton key={`skeleton-deal-activity-${index}`} />)}</ScrollArea></CardContent>
              </Card>
            </div>
                        ...
                        <div key={task.id} className={`p-3 border rounded-md hover:shadow-md transition-shadow bg-card ${task.completed ? 'opacity-60' : ''}`}>
                            <div className="flex justify-between items-start">
                                <div className="flex items-center">
                                    ...
                                    />
                                    <label htmlFor={`task-completed-${task.id}`} className={`font-medium text-sm ${task.completed ? 'line-through' : ''}`}>{task.title}</label>
                                </div>
                                <DropdownMenu>
