...
export async function PUT(request: NextRequest, { params }: { params: { companyId: string } }) {
  ...
  try {
    ...
    if (!organizationId || !userId) {
      console.error(
        `API PUT /api/companies/${companyId}: Unauthorized: Organization or User ID missing.`
      );
      return NextResponse.json(
    ...
    console.log(
      `API PUT /api/companies/${companyId} - Request body:`,
    ...
    const stmtCurrentCompany = db.prepare(
      "SELECT * FROM Companies WHERE id = ? AND organizationId = ?"
    );
    const currentCompanyData = stmtCurrentCompany.get(companyId, organizationId) as
      | Company
      | undefined;

    if (!currentCompanyData) {
      return NextResponse.json(
        { error: "Company not found or not authorized for update" },
        { status: 404 }
      );
    }
    ...
    if (result.changes === 0) {
      console.warn(
        `API PUT /api/companies/${companyId}: Company not found, not authorized, or no changes made`
      );
      return NextResponse.json(
        { error: "Company not found, not authorized, or no changes made" },
        { status: 404 }
      );
    }
    ...
    if (currentCompanyData.name !== name)
      changes.push({ field: "Name", oldValue: currentCompanyData.name, newValue: name });
    if ((currentCompanyData.industry || null) !== (industry === "_none_" ? null : industry || null))
      changes.push({
        field: "Industry",
        oldValue: currentCompanyData.industry,
        newValue: industry === "_none_" ? null : industry || null,
      });
    if ((currentCompanyData.website || null) !== (website || null))
      changes.push({
        field: "Website",
        oldValue: currentCompanyData.website,
        newValue: website || null,
      });
    if ((currentCompanyData.street || null) !== (street || null))
      changes.push({
        field: "Street",
        oldValue: currentCompanyData.street,
        newValue: street || null,
      });
    if ((currentCompanyData.city || null) !== (city || null))
      changes.push({ field: "City", oldValue: currentCompanyData.city, newValue: city || null });
    if ((currentCompanyData.state || null) !== (state || null))
      changes.push({ field: "State", oldValue: currentCompanyData.state, newValue: state || null });
    if ((currentCompanyData.postalCode || null) !== (postalCode || null))
      changes.push({
        field: "Postal Code",
        oldValue: currentCompanyData.postalCode,
        newValue: postalCode || null,
      });
    if ((currentCompanyData.country || null) !== (country || null))
      changes.push({
        field: "Country",
        oldValue: currentCompanyData.country,
        newValue: country || null,
      });
    if ((currentCompanyData.contactPhone1 || null) !== (contactPhone1 || null))
      changes.push({
        field: "Contact Phone 1",
        oldValue: currentCompanyData.contactPhone1,
        newValue: contactPhone1 || null,
      });
    if ((currentCompanyData.contactPhone2 || null) !== (contactPhone2 || null))
      changes.push({
        field: "Contact Phone 2",
        oldValue: currentCompanyData.contactPhone2,
        newValue: contactPhone2 || null,
      });
    if (
      (currentCompanyData.companySize || null) !==
      (companySize === "_none_" ? null : companySize || null)
    )
      changes.push({
        field: "Company Size",
        oldValue: currentCompanyData.companySize,
        newValue: companySize === "_none_" ? null : companySize || null,
      });
    if (
      (currentCompanyData.accountManagerId || null) !==
      (accountManagerId === "_none_" ? null : accountManagerId || null)
    )
      changes.push({
        field: "Account Manager ID",
        oldValue: currentCompanyData.accountManagerId,
        newValue: accountManagerId === "_none_" ? null : accountManagerId || null,
      });
    if ((currentCompanyData.description || null) !== (description || null))
      changes.push({
        field: "Description",
        oldValue: currentCompanyData.description,
        newValue: description || null,
      });
    if (JSON.stringify(currentCompanyData.tags || []) !== JSON.stringify(tags || []))
      changes.push({ field: "Tags", oldValue: currentCompanyData.tags, newValue: tags || [] });
    ...
    if (error.code && error.message) {
      ...
      if (error.code === "SQLITE_CONSTRAINT_FOREIGNKEY") {
        errorMessage =
          "Failed to update company due to a data conflict (e.g., invalid Account Manager ID).";
        statusCode = 409;
    ...
    console.error(
      `API PUT /api/companies/${params.companyId} - Responding with error:`,
      errorMessage,
      `Status: ${statusCode}`
    );
...
export async function DELETE(request: NextRequest, { params }: { params: { companyId: string } }) {
  ...
  try {
    ...
    if (!organizationId || !userId) {
      console.error(
        `API DELETE /api/companies/${companyId}: Unauthorized: Organization or User ID missing.`
      );
      return NextResponse.json(
        { error: "Unauthorized: Organization or User ID missing." },
        { status: 401 }
    ...
    const companyCheckStmt = db.prepare(
      "SELECT name FROM Companies WHERE id = ? AND organizationId = ?"
    );
    const companyToDeleteData = companyCheckStmt.get(companyId, organizationId) as
      | { name: string }
      | undefined;
    ...
    db.transaction(() => {
      const stmtUpdateContacts = db.prepare(
        "UPDATE Contacts SET companyId = NULL WHERE companyId = ? AND organizationId = ?"
      );
      stmtUpdateContacts.run(companyId, organizationId);

      const stmtUpdateDeals = db.prepare(
        "UPDATE Deals SET companyId = NULL WHERE companyId = ? AND organizationId = ?"
      );
      stmtUpdateDeals.run(companyId, organizationId);

      const stmtDeleteCompany = db.prepare(
        "DELETE FROM Companies WHERE id = ? AND organizationId = ?"
      );
