...
export async function DELETE(
  request: NextRequest,
  { params }: { params: { companyId: string; noteId: string } }
) {
  try {
    ...
    if (!companyData) {
      return NextResponse.json(
        { error: "Company not found or not authorized for note deletion" },
        { status: 404 }
      );
    }
    ...
    const noteStmt = db.prepare(
      "SELECT content FROM Notes WHERE id = ? AND companyId = ? AND organizationId = ?"
    );
    const noteToDelete = noteStmt.get(noteId, companyId, organizationId) as
      | { content: string }
      | undefined;

    const stmt = db.prepare(
      "DELETE FROM Notes WHERE id = ? AND companyId = ? AND organizationId = ?"
    );
    ...
    console.error(
      `API Error deleting note ${params.noteId} for company ${params.companyId}:`,
      error
    );
