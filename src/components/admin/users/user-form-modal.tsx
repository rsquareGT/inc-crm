...
export function UserFormModal({ isOpen, onClose, onSaveCallback, user, organizationId }: UserFormModalProps) {
  ...
  const onSubmit = async (data: UserFormData) => {
    ...
    try {
      ...
      if (isEditing && user?.id) { 
        ...
        toast({ title: "User Updated", description: `${data.firstName} ${data.lastName}'s details saved.` });
      } else { 
        ...
        toast({ title: "User Created", description: `New user ${data.firstName} ${data.lastName} added.` });
      }
      ...
      toast({ title: "Error Saving User", description: error instanceof Error ? error.message : "Could not save user.", variant: "destructive" });
    }
  };
      ...
      <DialogContent className="sm:max-w-lg">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 py-2 max-h-[70vh] overflow-y-auto pr-2">
          ...
          <div>
            ...
            <Input id="password" type="password" {...register('password')} placeholder={isEditing ? "Leave blank to keep current" : "Enter password"} disabled={isSubmitting}/>
            {errors.password && <p className="text-sm text-destructive mt-1">{errors.password.message}</p>}
          </div>
          <div>
            <Label htmlFor="profilePictureUrl">Profile Picture URL (optional)</Label>
            <Input id="profilePictureUrl" {...register('profilePictureUrl')} placeholder="https://example.com/image.png" disabled={isSubmitting}/>
            ...
            <div>
                <Controller
                    ...
                    <div className="flex items-center space-x-2 py-2">
                        <Checkbox id="isActive" checked={field.value} onCheckedChange={field.onChange} disabled={isSubmitting} />
                        <Label htmlFor="isActive" className="font-normal">User is Active</Label>
                    </div>
                    )}
                />
            </div>
