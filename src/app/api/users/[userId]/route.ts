...
export async function PUT(request: NextRequest, { params }: { params: { userId: string } }) {
  try {
    ...
    if (!requestingUserId || !requestingUserOrganizationId || !requestingUserRole) {
      console.warn(
        `API PUT /api/users/${targetUserId}: Unauthorized: User session details missing.`
      );
    ...
    const currentUserData = stmtCurrentUser.get(targetUserId, requestingUserOrganizationId) as
      | User
      | undefined;

    if (!currentUserData) {
      console.warn(
        `API PUT /api/users/${targetUserId}: User not found in organization ${requestingUserOrganizationId}`
      );
      return NextResponse.json(
        { error: "User not found or not authorized for this operation" },
        { status: 404 }
      );
    }
    ...
    if (targetUserId === requestingUserId) {
      if (firstName !== undefined && currentUserData.firstName !== firstName) {
        updates.firstName = firstName;
        changes.push({
          field: "First Name",
          oldValue: currentUserData.firstName,
          newValue: firstName,
        });
      }
      if (lastName !== undefined && currentUserData.lastName !== lastName) {
        updates.lastName = lastName;
        changes.push({
          field: "Last Name",
          oldValue: currentUserData.lastName,
          newValue: lastName,
        });
      }
      if (email !== undefined && currentUserData.email !== email) {
        ...
        const emailCheckStmt = db.prepare(
          "SELECT id FROM Users WHERE email = ? AND organizationId = ? AND id != ?"
        );
        const existingUserWithEmail = emailCheckStmt.get(
          email,
          requestingUserOrganizationId,
          targetUserId
        );
        if (existingUserWithEmail) {
          return NextResponse.json(
            { error: "This email address is already in use by another user in your organization." },
            { status: 409 }
          );
        }
        updates.email = email;
        changes.push({ field: "Email", oldValue: currentUserData.email, newValue: email });
      }
      if (
        profilePictureUrl !== undefined &&
        currentUserData.profilePictureUrl !== profilePictureUrl
      ) {
        updates.profilePictureUrl = profilePictureUrl;
        changes.push({
          field: "Profile Picture URL",
          oldValue: currentUserData.profilePictureUrl,
      ...
      if (
        ...
        console.warn(
          `API PUT /api/users/${targetUserId}: User trying to update restricted fields (role, isActive, organizationId) for self.`
        );
    ...
    // Admin editing a user
      ...
      if (role !== undefined && currentUserData.role !== role) {
        ...
        updates.role = role;
        changes.push({ field: "Role", oldValue: currentUserData.role, newValue: role });
      }

      if (
        isActive !== undefined &&
        typeof isActive === "boolean" &&
        currentUserData.isActive !== isActive
      ) {
        if (targetUserId === requestingUserId && !isActive) {
          // Admin trying to deactivate self
          const activeAdminCountStmt = db.prepare(
            "SELECT COUNT(*) as count FROM Users WHERE organizationId = ? AND role = 'admin' AND isActive = 1"
          );
          const adminCountResult = activeAdminCountStmt.get(requestingUserOrganizationId) as {
            count: number;
          };
          if (adminCountResult.count <= 1) {
            return NextResponse.json(
              { error: "Cannot deactivate the only active admin account in the organization." },
              { status: 403 }
            );
          }
        }
        ...
        changes.push({
          field: "Status",
          oldValue: currentUserData.isActive ? "Active" : "Inactive",
    ...
    // Not admin and not self-editing
      ...
      console.warn(
        `API PUT /api/users/${targetUserId}: Unauthorized attempt to update user by ${requestingUserId}.`
      );
      return NextResponse.json(
    ...
    if (Object.keys(updates).length === 0) {
      return NextResponse.json(
        { message: "No changes to apply.", user: currentUserData },
        { status: 200 }
      );
    }
    ...
    if (result.changes === 0) {
      console.warn(
        `API PUT /api/users/${targetUserId}: Update failed or no rows affected for user in organization ${requestingUserOrganizationId}.`
      );
      return NextResponse.json(
        { error: "User not found or no changes made during update" },
        { status: 404 }
      );
    }
    ...
    const stmtUpdatedUser = db.prepare(
      "SELECT id, organizationId, email, firstName, lastName, profilePictureUrl, role, isActive, createdAt, updatedAt FROM Users WHERE id = ? AND organizationId = ?"
    );
    const updatedUserData = stmtUpdatedUser.get(targetUserId, requestingUserOrganizationId) as User;

    console.log(
      `API PUT /api/users/${targetUserId}: User updated successfully by ${requestingUserId}.`
    );
