...
const getActivityIcon = (
  ...
  switch (entityType) {
    ...
    case "task":
      return <ListChecks className="h-4 w-4 text-lime-500" />;
    case "note":
      return <StickyNote className="h-4 w-4 text-slate-500" />;
    case "organization":
      return <OrgIcon className="h-4 w-4 text-purple-500" />;
    case "user":
      return <UserRound className="h-4 w-4 text-teal-500" />; // Changed from ShieldAlert
    default:
...
const UserXIcon = (
  props: React.SVGProps<SVGSVGElement> // Kept for UserXIconLucide name conflict, though not directly used by name
) => (
  <svg
...
const getActivityDescription = (activity: Activity): React.ReactNode => {
  ...
  if (activity.entityType === "organization") {
    ...
    entityDisplayName = activity.entityName
      ? `organization "${activity.entityName}"`
  ...
  } else if (activity.entityType === "contact" || activity.entityType === "deal") {
    // For 'contact', 'deal'
  ...
  const mainActionText = () => {
    switch (activity.activityType) {
      case "created_contact":
        return (
          <>
            <span className="font-medium">{userName}</span> created {entityLink}.
          </>
        );
      case "updated_contact":
        return (
          <>
            <span className="font-medium">{userName}</span> updated {entityLink}.
          </>
        );
      case "deleted_contact":
        return (
          <>
            <span className="font-medium">{userName}</span> deleted contact &quot;
      ...
      case "deleted_company":
        return (
          <>
            <span className="font-medium">{userName}</span> deleted company "
            {activity.entityName || "N/A"}".
          </>
        );
      ...
      case "updated_deal_details":
        return (
          <>
            <span className="font-medium">{userName}</span> updated details for {entityLink}.
          </>
        );
      case "updated_deal_stage":
        return (
          <>
            ...
            {activity.details?.old_stage && (
              <>
                {" "}
                from <span className="font-semibold">{activity.details.old_stage}</span>
              </>
            )}
            {activity.details?.new_stage && (
              <>
                {" "}
                to <span className="font-semibold">{activity.details.new_stage}</span>
              </>
            )}
            .
          </>
      ...
      case "deleted_deal":
        return (
          <>
            <span className="font-medium">{userName}</span> deleted deal "
            {activity.entityName || "N/A"}".
          </>
        );
      ...
      case "completed_task":
        return (
          <>
            <span className="font-medium">{userName}</span> completed {entityLink}.
          </>
        );
      case "deleted_task":
        return (
      ...
      case "activated_user":
        return (
          <>
            <span className="font-medium">{userName}</span> activated {entityLink}.
          </>
        );
      case "deactivated_user":
        return (
          <>
            <span className="font-medium">{userName}</span> deactivated {entityLink}.
          </>
        );
      ...
      case "added_note_to_deal":
        return (
          <>
            ...
            {activity.details?.noteContentPreview && (
              <>
                :{" "}
                <span className="italic text-muted-foreground">
                  "{String(activity.details.noteContentPreview).substring(0, 50)}..."
                </span>
              </>
            )}
            .
      ...
      default:
        return (
          <>
            <span className="font-medium">{userName}</span> performed an action on {entityLink}.
          </>
        );
    }
  };

  const changes = activity.details?.changes as
          ...
          {changes.map((change, index) => (
            <li key={index}>
              ...
              {change.oldValue !== undefined &&
                ...
                ` changed from "${String(change.oldValue).substring(0, 50)}${String(change.oldValue).length > 50 ? "..." : ""}" to "${String(change.newValue).substring(0, 50)}${String(change.newValue).length > 50 ? "..." : ""}"`}
              {change.oldValue === undefined &&
                change.newValue !== undefined &&
                ` set to "${String(change.newValue).substring(0, 50)}${String(change.newValue).length > 50 ? "..." : ""}"`}
              {change.oldValue !== undefined &&
                change.newValue === undefined &&
                ` cleared (was "${String(change.oldValue).substring(0, 50)}${String(change.oldValue).length > 50 ? "..." : ""}")`}
  ...
  return (
    <div className="flex items-start space-x-3 py-3 border-b border-border/50 last:border-b-0 hover:bg-secondary/30 transition-colors duration-150 px-2 rounded-md">
      ...
      <div className="flex-1 text-sm">
        <div className="text-foreground leading-tight">{getActivityDescription(activity)}</div>
