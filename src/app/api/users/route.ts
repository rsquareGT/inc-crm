...
async function getAdminFromRequest(request: NextRequest): Promise<{ id: string, organizationId: string, role: UserRole, email: string } | null> {
  if (!JWT_SECRET) {
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    console.log(`API GET /api/users: Admin ${admin.email} (Org: ${admin.organizationId}) fetching users.`);
    ...
    const stmtUsers = db.prepare('SELECT id, organizationId, email, firstName, lastName, profilePictureUrl, role, isActive, createdAt, updatedAt FROM Users WHERE organizationId = ? ORDER BY lastName ASC, firstName ASC');

    const usersData = stmtUsers.all(admin.organizationId) as User[];
    console.log(`API GET /api/users: Found ${usersData.length} users for organization ${admin.organizationId}.`);
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.log(`API POST /api/users: Admin ${admin.email} (Org: ${admin.organizationId}) attempting to create user.`);
    ...
    console.log(`API POST /api/users: User ${email} created successfully with ID ${newUserId} in organization ${admin.organizationId}.`);
    ...
    if (error.code === 'SQLITE_CONSTRAINT_UNIQUE' && error.message.includes('Users.email')) {
        console.warn(`API POST /api/users: Attempt to create user with existing email: ${error.message}`);
