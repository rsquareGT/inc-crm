...
export function CompanyCard({ company, allUsers, onEdit, onDelete }: CompanyCardProps) {
  ...
  const accountManager = company.accountManagerId
    ? allUsers.find((u) => u.id === company.accountManagerId)
    : undefined;
      ...
      <CardContent className="px-4 pb-3 space-y-2 text-sm flex-grow">
        ...
        {company.companySize && (
          <div className="flex items-center text-muted-foreground">
            ...
            <span className="truncate" title={company.companySize}>
              {company.companySize}
            </span>
          </div>
        )}
        ...
        {company.description && (
          <div className="flex items-start text-muted-foreground pt-1">
            ...
            <p className="text-xs line-clamp-2" title={company.description}>
              {company.description}
            </p>
          </div>
        )}
