      ...
      {navItems.map((item) => (
        <Link
          ...
          className={cn(
            ...
            pathname === item.href || (item.href !== '/dashboard' && pathname.startsWith(item.href)) || (item.href === '/dashboard' && pathname === '/')
              ? 'text-primary bg-primary/10'
              : 'text-muted-foreground hover:text-foreground/80'
          )}
        >
          <item.icon className="h-4 w-4 flex-shrink-0" />
          <span className="hidden sm:inline-block whitespace-nowrap">{item.label}</span> {/* Hide label on xs screens, ensure no wrap */}
        </Link>
      ))}
    </nav>
  );
