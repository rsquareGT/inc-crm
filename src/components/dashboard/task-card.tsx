...
export function TaskCard({
  ...
  const getRelatedItemName = (): React.ReactNode => {
    if (task.relatedDealId) {
      ...
      return deal ? (
        <Link
          href={`/deals/${deal.id}`}
          className="hover:underline text-primary text-xs flex items-center"
        >
    ...
    if (task.relatedContactId) {
      ...
      return contact ? (
        <Link
          ...
          className="hover:underline text-primary text-xs flex items-center"
        >
          <UserCircle className="h-3 w-3 mr-1 flex-shrink-0" /> {contact.firstName}{" "}
          {contact.lastName}
        </Link>
      ) : (
  ...
  return (
    <Card
      className={cn(
        "mb-3 shadow-md hover:shadow-lg transition-shadow",
    ...
    >
      ...
      <CardContent className="px-4 pb-2 space-y-1.5 text-sm">
        ...
        <div className="text-xs text-muted-foreground">Related to: {getRelatedItemName()}</div>
        {task.description && (
      ...
      {task.tags && task.tags.length > 0 && (
        <CardFooter className="px-4 pt-1 pb-3 flex flex-wrap gap-1">
          {task.tags.slice(0, 3).map((tag) => (
            <TagBadge key={tag} tag={tag} className="text-xs px-1.5 py-0.5" />
          ))}
          {task.tags.length > 3 && (
